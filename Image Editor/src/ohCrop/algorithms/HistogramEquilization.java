...
//	1. Calculate histogram of the grayscale values in the source image. For this first part of the
...
//	3. Calculate an idealized histogram by dividing the number of pixels in your image with the
//	number of bins in your histogram.
//	4. Calculate the cumulative frequency distribution of the idealized histogram.
//	5. Design the mapping comparing the cumulative frequency distribution of the idealized histogram
...
//	6. Map each pixel of the source image to the new intensity value.
	 ...
	 * @param imageRaster The image that is being equalized.
	...
	public static int[] calculateHistogram(int[] imageRaster) {
		...
		for(int i = 0; i < imageRaster.length; i++) {
			...
			//TODO: UNCOMMENT THIS ^
			//int currentValue = imageRaster[i];
			int myBin = currentValue;
			bins[myBin]++;
		}
	 ...
	 * @param bins The given histogram.
	 ...
	 * @param histogram The histogram to be converted.
	 * @param pixelCount The amount of pixels in the image.
	...
	/**
	 * Maps values from the original frequency distribution to the cumulative frequency distribuition.
	 * @param cuf The original frequency distribution. 
	 * @param cuFeq THe frequency distribution of the of the idealized histogram.
	 ...
	 * @param mapping The mapping used to create the new image.
	 * @param original The original image. 
	 * @return The new image.
	 */
	public static int[] mapPixels(int[] mapping, int[] original) {
		int[] newImage = new int[original.length];
		
		for(int i = 0; i < original.length; i++) {
			int currentValue = (original[i] & RED_MASK) >> RED_OFFSET;
			newImage[i] = mapping[currentValue];
		}
